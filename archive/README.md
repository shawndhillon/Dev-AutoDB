...
- [minikube](https://minikube.sigs.k8s.io/docs/start/) or [kind](https://kind.sigs.k8s.io/docs/user/quick-start/) or [k3d](https://k3d.io/stable/#quick-start)
...
## 0. Build Custom Image (Required)

The Kubernetes manifests require a custom PostgreSQL image with replication setup:

```bash
# Build from archive directory
docker build -t postgres:16-alpine .

# For minikube, load into minikube's Docker daemon:
minikube image load postgres:16-alpine

# For kind, load into kind cluster:
kind load docker-image postgres:16-alpine

# For k3d, import into k3d:
k3d image import postgres:16-alpine -c <cluster-name>
```
